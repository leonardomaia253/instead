...
function StepReview({
  ...
  return (
    <div>
      ...
      <p style={styles.stepDesc}>Verifique todos os parâmetros antes de fazer o deploy.</p>
      ...
      {!!feeInEth && (
        <div style={{
          ...
          <span style={{ fontWeight: 700 }}>Taxa de Criação</span>
          <span style={{ fontSize: 22, fontWeight: 700, color: "var(--accent-1)" }}>
      ...
      <button
        ...
        style={{ width: "100%", marginTop: 24, padding: "16px 0" }}
      >
        {isPending ? "⏳ Aguardando..." :
          isConfirming ? "⛓️ Confirmando..." :
            isConfirmed ? "✅ Lançado!" :
              "🚀 Fazer Deploy"}
      </button>
    </div>
...
function CharCount({ current, max }: { current: number; max: number }) {
  return <div style={{ textAlign: "right", fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>{current}/{max}</div>;
}
...
function InfoBox({ children, color }: { children: React.ReactNode; color: "blue" | "purple" | "red" }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 12, padding: 16, marginTop: 24, fontSize: 13, color: "var(--text-muted)",
    }}>
      {children}
    </div>
  );
...
const styles = {
  stepTitle: { fontSize: 26, fontWeight: 700, marginBottom: 8 } as React.CSSProperties,
  stepDesc: { fontSize: 15, color: "var(--text-muted)", marginBottom: 8 } as React.CSSProperties,
};
...
export default function FactoryPage() {
  ...
  useEffect(() => {
    if (isConfirmed) setShowOnboarding(true);
  }, [isConfirmed]);
  ...
  async function handleDeploy() {
    ...
    try {
      ...
      await insertAudit({
        user_wallet: address,
        action: "CREATE_TOKEN",
        metadata: { name: form.name, tx_hash: hash, chain_id: form.chainId }
      });
    } catch (e) { }
  }
    ...
    <main style={{ minHeight: "100vh", padding: "32px 24px" }}>
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 36 }}>
          <h1 style={{ fontSize: 30, fontWeight: 700 }}>🏭 Token Factory</h1>
          <ConnectButton />
        </div>
        ...
        <div className="card">
          ...
          {step < 5 && (
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 32 }}>
              {step > 1 && <button className="btn-outline" onClick={() => setStep(step - 1)}>Voltar</button>}
      ...
      <AIAssistant
        type="token"
        contextData={{ name: form.name, step }}
      />
    </main>
  );
}